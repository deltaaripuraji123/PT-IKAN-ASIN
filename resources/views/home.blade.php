...
<div class="bg-gradient-to-r from-teal-500 to-blue-700 text-white py-16 shadow-lg">
    <div class="container mx-auto px-4 text-center">
        <h1 class="text-4xl font-bold mb-4 tracking-wide">Selamat Datang di Ikan Asin Store</h1>
        <p class="text-lg mb-8 max-w-2xl mx-auto">Temukan berbagai jenis ikan asin berkualitas tinggi langsung dari nelayan terpercaya</p>
    ...
    <div class="mb-20">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="group bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
                <div class="h-56 bg-gray-50 flex items-center justify-center relative overflow-hidden">
                    ...
                    <i class="fas fa-fish text-6xl text-teal-500 group-hover:text-teal-600 transition-colors"></i>
                </div>
                <div class="p-6">
                    <h3 class="text-xl font-bold mb-3 text-gray-800">{{ $category->name }}</h3>
                    <a href="{{ route('products.index', ['category' => $category->name]) }}" class="inline-flex items-center text-sm text-gray-600 font-medium hover:text-teal-700 group-hover:translate-x-1 transition-transform">
                        Lihat Produk 
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="group bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
                <a href="{{ route('products.show', $product->id) }}" class="block relative overflow-hidden">
                    <div class="h-56 bg-gray-50 flex items-center justify-center">
                        @if($product->image)
                    ...
                    <div class="absolute top-3 right-3 bg-red-400 text-white text-xs font-semibold px-2 py-0.5 rounded">
                        Hot Deal
                    </div>
                </a>
                <div class="p-4">
                    <h3 class="text-base font-bold mb-1 text-gray-800 group-hover:text-teal-600 transition-colors">{{ $product->name }}</h3>
                    <p class="text-lg font-bold text-teal-600 mb-2">Rp. {{ number_format($product->price, 0, ',', '.') }}</p>
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
