<header class="bg-white shadow" x-data="{ userMenuOpen: false, mobileMenuOpen: false }">
    <nav class="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <div class="hidden md:flex space-x-6">
            ...
            <a href="{{ route('products.index') }}" class="text-gray-700 hover:text-blue-600 transition-colors">Produk</a>
        </div>
        ...
        <div class="flex items-center space-x-4">
            ...
            @guest
                ...
                <a href="{{ route('register') }}" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">Daftar</a>
            @else
                <!-- User Dropdown -->
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = ! open" @click.away="open = false" class="flex items-center space-x-3 text-gray-700 hover:text-blue-600 transition-colors">
                        <!-- User Avatar -->
                    ...
                    <div x-show="open" 
                        ...
                        <div class="py-2">
                            @if (auth()->user()->isAdmin())
                                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                    <i class="fas fa-tachometer-alt w-4"></i>
                                    Admin Dashboard
                                </a>
                            @endif
                            <a href="{{ route('order.history') }}" class="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="flex items-center gap-3 w-full px-4 py-2 text-sm text-left text-red-600 hover:bg-gray-100">
